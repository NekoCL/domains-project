...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld) payable ERC721("Asuka Name Service", "ANS") {
    ...
    function price(string calldata name) public pure returns(uint) {
        ...
        require(len > 0);
        if (len == 3) {
            return 5 * 10**15; // 0.005 MATIC (MATIC has 18 decimals)
        } else if (len == 4) {
            return 3 * 10**15;
        } else {
            return 1 * 10**15;
    ...
    // A register function that adds names to mapping
    function register(string calldata name) public payable {
        ...
        // Create SVG image for NFT with the name
        ...
        // Create the JSON metadata of the NFT combining strings and encoding as base64
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        _name,
                        '", "description": "A domain on the Asuka Name Service", "image": "data:image/svg+xml;base64,',
